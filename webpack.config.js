...
module.exports = config => {
    ...
    _.merge(
      {
        // payload settings
        watchPaths: [
          path.resolve(`${__dirname}/${DM.ad.get().settings.source.path}`)
        ],
        entries: [
          {
            name: "inline",
  ...
  const buildNodeModulesAliases = DM.aliases.getTopLevel(
    path.resolve(context, "node_modules/@ff0000-ad-tech")
  );

  // build.js path
  const buildEntry = path.resolve(
    __dirname,
    `${DM.deploy.get().source.context}/${DM.deploy.get().source.size}/build.js`
  );
      ...
      alias: Object.assign(
        {
          AdData: path.resolve(
            __dirname,
            `${DM.deploy.get().source.context}/common/js/data/AdData`
          ),
          FtData: path.resolve(
            __dirname,
            `${DM.deploy.get().source.context}/common/js/data/FtData`
          ),
          GdcData: path.resolve(
            __dirname,
            `${DM.deploy.get().source.context}/common/js/data/GdcData`
          ),
          "@common": path.resolve(
            __dirname,
